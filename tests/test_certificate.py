...
SIGNER = (
    "0x" + cry.public_key_to_address(cry.secp256k1.derive_publicKey(PRIV_KEY)).hex()
)
...
def test_encode():
    ...
    sig_bytes = cry.secp256k1.sign(
        cry.blake2b256([certificate.encode(cert).encode("utf-8")])[0], PRIV_KEY
    )
...
def test_verify():
    to_be_signed, _ = cry.blake2b256([certificate.encode(cert).encode("utf-8")])

    sig_bytes = cry.secp256k1.sign(to_be_signed, PRIV_KEY)
